#!/usr/bin/env python3
"""Borda restricted to instances where the AI chose cpsat.

Predictions are reconstructed deterministically by loading each LOYO model
and running it on the training-data feature matrix for the held-out year.
For every (year, problem, name) where the AI picked class 0 (cpsat), we
compare the AI's run against the cpsat8 baseline using utils.borda._compare.
"""
...
OUT_CSV          = ROOT / "leaderboard_when_cpsat.csv"
OUT_TYP          = ROOT / "leaderboard_when_cpsat.typ"
OUT_INSTANCES    = ROOT / "cpsat_predicted_instances.csv"
...
def main():
    ...
    for sched, family in AIS:
        for year in YEARS:
            ...
            n_cpsat = sum(1 for v in predicted[(sched, year)].values() if v == 0)
            n_total = len(predicted[(sched, year)])
            print(f"{sched} {year}: predicted cpsat on {n_cpsat}/{n_total} "
                  f"({n_cpsat/max(n_total,1)*100:.1f}%) of features-available instances")
    print()

    inst_rows = []
    rows_out = []
    print(f"{'ai':<10} {'year':<6} {'n_cpsat_pick':>13} "
        ...
        for year in YEARS:
            ...
            for (sched_k, y, p, n), ai_r in ai_rows.items():
                ...
                inst_rows.append({
                    ...
                    "delta_ms":    int(ai_r["time_ms"]) - int(cp["time_ms"]),
                    "delta_s":     round((ai_r["time_ms"] - cp["time_ms"]) / 1000.0, 2),
                    "borda_ai":    round(sa, 4),
    ...
    with open(OUT_CSV, "w", newline="") as f:
        w = csv.DictWriter(f, fieldnames=[
            "ai", "year", "n_cpsat_pick", "borda_ai", "borda_cpsat", "normalized",
        ])
        w.writeheader()
        w.writerows(rows_out)
    print(f"Wrote -> {OUT_CSV.relative_to(ROOT)}")

    inst_rows.sort(key=lambda r: (-r["delta_ms"], r["ai"], r["year"]))
    with open(OUT_INSTANCES, "w", newline="") as f:
        w = csv.DictWriter(f, fieldnames=[
            "ai", "year", "problem", "model", "name",
            "ai_status", "ai_time_ms", "ai_obj",
            "cp_status", "cp_time_ms", "cp_obj",
            "delta_ms", "delta_s", "borda_ai", "borda_cpsat",
        ])
        w.writeheader()
        w.writerows(inst_rows)
    print(f"Wrote -> {OUT_INSTANCES.relative_to(ROOT)} "
          f"(sorted by delta_ms desc; biggest AI-vs-cpsat slowdowns first)")
    ...
    typ.append("      [AI], [Year], [n cpsat-picked], "
    ...
    for i, r in enumerate(rows_out):
        ...
        typ.append(
            f"    [{r['ai']}], [{year_label}], [{r['n_cpsat_pick']}], "
    ...
    typ.append("  caption: ["
               ...
               "predicted cpsat. On these instances the AI's executed "
               "schedule is functionally identical to the cpsat8 baseline "
               "except for the seconds spent on the no-yuck-equivalent "
               "static portfolio while features are being extracted, so any "
               "shortfall here measures the feature-extraction overhead "
               "rather than a wrong portfolio choice. Per-instance "
               "maximum is 1."
               "],")
    typ.append("  ) <tab:vs-cpsat-when-cpsat>")
    typ.append("")
    OUT_TYP.write_text("\n".join(typ))
