"""
Ablation V1: argmax-label CE, no ensemble, val-CE early stopping.
Same nested CV protocol as nn.py (the V3/current variant).
"""
import os
...
def train_mlp(X_tr, y_tr, num_classes, params):
    ...
    yt = _to_tensor(y_tr[tr_idx], torch.long)
    Xv = _to_tensor(X_tr[val_idx], torch.float32)
    yv = _to_tensor(y_tr[val_idx], torch.long)
...
def cv_score(X, y_labels, Y_borda, num_classes, params, splits):
    fold_means = []
    for tr, te in splits:
        scaler = StandardScaler().fit(X[tr])
        Xtr = scaler.transform(X[tr]).astype(np.float32)
        Xte = scaler.transform(X[te]).astype(np.float32)
        model = train_mlp(Xtr, y_labels[tr], num_classes, params)
...
def run_hpo(X, y_labels, Y_borda, groups, num_classes, n_splits, n_trials):
    ...
    def objective(trial):
        ...
        return cv_score(X, y_labels, Y_borda, num_classes, params, splits)
...
def main():
    print("========== ablation V1: argmax CE, no ensemble, val-CE ES ==========")
    ...
    for fold_label, train_idx, test_idx in folds:
        ...
        model = train_mlp(Xtr_s, y_labels[train_idx], num_classes, best_params)
