"""
SVC variant of svm.py that weights each training instance by its Borda
margin |Y_borda[:,1] - Y_borda[:,0]|: confident labels pull the decision
boundary, near-ties contribute little. Same nested CV, same (C, gamma) HPO.
"""
import os
...
def _one_inner_fold(X, y_labels, Y_borda, margin, params, tr, te):
    pipe = make_pipeline(**params)
    pipe.fit(X[tr], y_labels[tr], model__sample_weight=margin[tr])
    pred = pipe.predict(X[te])
    bordas = Y_borda[te][np.arange(len(te)), pred]
    return bordas.mean()


def cv_score(X, y_labels, Y_borda, margin, params, splits):
    fold_means = Parallel(n_jobs=INNER_JOBS, prefer="threads")(
        delayed(_one_inner_fold)(X, y_labels, Y_borda, margin, params, tr, te)
        for tr, te in splits
    )
    return float(np.mean(fold_means))


def run_hpo(X, y_labels, Y_borda, margin, groups, n_splits, n_trials):
    ...
    def objective(trial):
        ...
        return cv_score(X, y_labels, Y_borda, margin, params, splits)
...
def _evaluate_one_fold(X, y_labels, Y_borda, margin, years,
...
def evaluate_dataset(name, getter):
    ...
    margin = np.abs(Y_borda[:, 1] - Y_borda[:, 0])
    years = meta["year"]
    ...
    fold_records = Parallel(n_jobs=OUTER_JOBS)(
        delayed(_evaluate_one_fold)(X, y_labels, Y_borda, margin, years,
                                    fold_label, train_idx, test_idx)
    ...
    final_params, final_score = run_hpo(
        X, y_labels, Y_borda, margin, years,
        n_splits=INNER_K, n_trials=N_TRIALS,
    )
    final_pipe = make_pipeline(**final_params)
    final_pipe.fit(X, y_labels, model__sample_weight=margin)

    Path("models").mkdir(exist_ok=True)
    out_path = Path(f"models/svc_sample_weight_{name}.joblib")
