"""
SVC + threshold fallback on cpsat8_k1. Mirrors svm.py but SVC uses
probability=True (Platt-calibrated); decision rule predicts argmax(proba)
with cpsat8 fallback when max(proba) < threshold. Threshold is the third
Optuna HP (uniform [0.5, 0.99]).
"""
import os
...
def threshold_predict(pipe, X, threshold):
...
def _one_inner_fold(X, y_labels, Y_borda, params, threshold, tr, te):
    pipe = make_pipeline(**params)
    pipe.fit(X[tr], y_labels[tr])
    pred = threshold_predict(pipe, X[te], threshold)
    bordas = Y_borda[te][np.arange(len(te)), pred]
    return bordas.mean()


def cv_score(X, y_labels, Y_borda, params, threshold, splits):
    fold_means = Parallel(n_jobs=INNER_JOBS, prefer="threads")(
        delayed(_one_inner_fold)(X, y_labels, Y_borda, params, threshold, tr, te)
        for tr, te in splits
    )
...
def run_hpo(X, y_labels, Y_borda, groups, n_splits, n_trials):
    ...
    def objective(trial):
        ...
        return cv_score(X, y_labels, Y_borda, params, threshold, splits)
...
def _evaluate_one_fold(X, y_labels, Y_borda, years, fold_label, train_idx, test_idx):
    ...
    pred = threshold_predict(final, X[test_idx], threshold)
...
def evaluate_dataset(name, getter):
    ...
    for r in fold_records:
        ...
        print(f"    {r['fold_label']}: borda={r['test_borda']:>6.2f}  "
              ...
              f"t={r['threshold']:.2f}, {r['fit_seconds']:.0f}s)")
    ...
    thrs = [r["threshold"] for r in fold_records]
    print(f"  thresholds per fold: mean={np.mean(thrs):.3f}  "
          f"range=[{min(thrs):.3f}, {max(thrs):.3f}]")
    ...
    final_threshold = final_params.pop("threshold")
    final_pipe = make_pipeline(**final_params)
    final_pipe.fit(X, y_labels)

    Path("models").mkdir(exist_ok=True)
    out_path = Path(f"models/svc_threshold_{name}.joblib")
    joblib.dump({"model": final_pipe, "threshold": float(final_threshold)}, out_path)
    print(f"  saved {out_path}  (params: C={final_params['C']:.3g}, "
          f"gamma={final_params['gamma']:.3g}, threshold={final_threshold:.3f},  "
          f"cv_score={final_score:.4f})")

    return fold_records, final_params, final_threshold, final_score
